...
class SlackAdapter(BasePlatformAdapter):
    ...
    async def _handle_slack_message(
        ...
        msg_user = event.get("user", "") or (event.get("bot_profile") or {}).get(
            "user_id", ""
        )
        ...
        if sender_is_bot:
            ...
            if allow_bots == "none" and not is_free_response_bot_message:
                ...
                mentioned_usergroups = set(
                    re.findall(r"<!subteam\^([A-Z0-9]+)", text_check)
                )
                name_addressed = (
                    not mentioned_users
        ...
        if user_id and user_id != bot_uid:
            ...
            if sender_is_bot_user:
                ...
                if allow_bots == "none":
                    return
                if allow_bots == "mentions" and not directly_addressed:
                    return
