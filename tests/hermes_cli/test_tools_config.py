...
def test_slack_plugin_toolset_does_not_leak_to_api_server(monkeypatch):
    """A lazily discovered current-thread Slack tool is Slack-only."""
    import hermes_cli.tools_config as tools_config

    monkeypatch.setattr(tools_config, "_get_plugin_toolset_keys", lambda: {"slack"})
    config = {
        "platform_toolsets": {"api_server": ["hermes-api-server", "slack"]},
        "known_plugin_toolsets": {"api_server": ["slack"]},
    }

    assert "slack" not in _get_platform_tools(config, "api_server")
