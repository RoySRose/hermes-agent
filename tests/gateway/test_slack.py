...
class TestSlackSocketWatchdog:
    ...
    # -- closed aiohttp session / ping-pong staleness: heals wedged Socket Mode --
    ...
    def test_client_session_closed_when_aiohttp_session_closed(self):
        session = MagicMock(closed=True)
        adapter = self._adapter_with_fake_client(aiohttp_client_session=session)
        assert adapter._socket_client_session_closed() is True

    def test_client_session_not_closed_without_aiohttp_backend(self):
        adapter = self._adapter_with_fake_client()
        assert adapter._socket_client_session_closed() is False
    ...
    @pytest.mark.asyncio
    async def test_watchdog_reconnects_when_aiohttp_client_session_is_closed(self):
        adapter = SlackAdapter(PlatformConfig(enabled=True, token="xoxb-fake"))
        adapter._socket_watchdog_interval_s = 0.01
        factory, instances = self._make_fake_handler_factory()
        ...
        with contextlib.ExitStack() as stack:
            ...
            try:
                assert await adapter.connect() is True
                assert len(instances) == 1
                instances[0].client.aiohttp_client_session = MagicMock(closed=True)
                ...
                assert len(instances) >= 2, "watchdog did not rebuild closed aiohttp session"
                assert instances[0].closed is True
                assert adapter._handler is instances[-1]
            finally:
                await adapter.disconnect()
    ...
    async def test_watchdog_reconnects_when_ping_pong_stale_despite_is_connected_true(self):
        ...
        with contextlib.ExitStack() as stack:
            ...
            try:
                ...
                assert len(instances) >= 2, "watchdog did not heal wedged transport"
                assert instances[0].closed is True
                assert adapter._handler is instances[-1]
            finally:
                await adapter.disconnect()
