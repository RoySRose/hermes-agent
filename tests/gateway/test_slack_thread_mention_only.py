...
def _call_mode_tool(action: str, *, platform: str = "slack") -> dict:
    ...
    try:
        ...
        # reset (not clear): clear_session_vars() binds "" — an "explicitly
        # cleared" value that suppresses the os.environ fallback and survives
        # into later tests in this thread, where it resolves the approval
        # session key to "" and silently takes the no-gateway-callback path.
        reset_session_vars()
...
async def test_recovered_gap_reaches_gateway_model_input_as_context_block():
    ...
    # This tree suffixes the Slack sender name with a verifiable user-id
    # reference (#17916, gateway/run.py _prepare_inbound_message_text) —
    # unrelated to mention-only gap recovery; the OSI source predates that
    # feature and asserted the bare "[tester]" prefix.
    assert (
        "[New message]\n[tester | Slack user <@U_USER>] 그 내용 반영해서 답해줘"
        in model_input
    )
